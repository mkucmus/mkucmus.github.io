...
const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: "ListingFiltersManufacturer",
  ...
  setup(__props, { emit: emits }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.filter.entities, (entity) => {
        var _a;
        _push(`<div class="flex items-center"><input${ssrRenderAttr("id", `filter-mobile-${__props.filter.code}-${entity.id}`)}${ssrIncludeBooleanAttr((_a = unref(selectedOptionIds)) == null ? void 0 : _a.includes(entity.id)) ? " checked" : ""}${ssrRenderAttr("name", __props.filter.name)}${ssrRenderAttr("value", entity.name)} type="checkbox" class="h-4 w-4 border-gray-300 rounded text-indigo-600 focus:ring-indigo-500"><label${ssrRenderAttr("for", `filter-mobile-${__props.filter.code}-${entity.id}`)} class="ml-3 min-w-0 flex-1 text-gray-500">${ssrInterpolate(entity.name)}</label></div>`);
      });
      _push(`<!--]--></div></div></div>`);
    };
  }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/listing-filters/ListingFiltersManufacturer.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=ListingFiltersManufacturer-d9b68bb4.mjs.map
